...
| -------------- | ----------- | ----------------------- |
| `BORRADOR`     | Cita creada por el paciente sin enviar para revisión. | `BORRADOR` → `PENDIENTE` (paciente envía), `BORRADOR` → `CANCELADA` (paciente cancela). |
| `PENDIENTE`    | Cita enviada para revisión del nutricionista. | `PENDIENTE` → `APROBADA` (nutricionista), `PENDIENTE` → `RECHAZADA` (nutricionista), `PENDIENTE` → `CANCELADA` (paciente). |
| `APROBADA`     | Cita confirmada por el nutricionista o creada directamente por nutricionista/administrador. Inmutable para el paciente. | `APROBADA` → `COMPLETADA` (automático/post consulta), `APROBADA` → `CANCELADA` (nutricionista/paciente vía soporte). |
...
1. **Creación**
   ...
   - Nutricionistas y administradores pueden crear citas directamente en `APROBADA` cuando coordinan la agenda por otros canales.
2. **Solicitud de revisión**
   - El paciente envía la cita (`BORRADOR` → `PENDIENTE`).
   - Se notifica al nutricionista.
3. **Revisión del nutricionista**
   - El nutricionista acepta (`PENDIENTE` → `APROBADA`) o rechaza (`PENDIENTE` → `RECHAZADA`).
...
4. **Ajustes posteriores al rechazo**
   - El paciente revisa el comentario y decide:
     - Ajustar y reenviar (`RECHAZADA` → `BORRADOR` → `PENDIENTE`).
     - Cancelar definitivamente (`RECHAZADA` → `CANCELADA`).
5. **Post aprobación**
   - Mientras la cita esté `APROBADA`, el paciente solo puede consultar y cancelar (si las políticas lo permiten). No puede modificar la fecha.
   - Una vez atendida, el sistema la marca como `COMPLETADA`.
   - Los cambios que impliquen mover o cancelar una cita `APROBADA` deben ser gestionados por el nutricionista o soporte administrativo.
6. **Historial**
...
- **Pacientes**
  ...
  - Una vez enviada (`PENDIENTE`), los cambios implican volver a `BORRADOR`; limitar a **2 reversiones** por cita dentro de una ventana de **7 días**.
  ...
  - Solo pueden cancelar hasta **24 horas antes** de la hora agendada cuando la cita esté `APROBADA`; después de ese límite deben contactar al nutricionista.
- **Nutricionistas**
  - Pueden mover o cancelar citas `APROBADA` hasta 2 horas antes de la cita; se recomienda registrar el motivo en historial.
  - No tienen límite de modificaciones mientras la cita permanezca en `PENDIENTE` o `RECHAZADA`.
- **Administradores**
...
| ------ | ------------- | ----- |
| Crear cita | `POST /pacientes/:pacienteId/citas` | Requiere validar disponibilidad. Devuelve estado `BORRADOR` o `PENDIENTE` según el flujo de UX. |
| Editar cita | `PATCH /citas/:id` | Solo permitido para estados `BORRADOR` o `RECHAZADA`. Requiere auditoría. |
| Enviar a revisión | `POST /citas/:id/enviar` | Cambia de `BORRADOR` a `PENDIENTE`. Validar límite de reversiones semanales. |
| Cancelar cita | `POST /citas/:id/cancelar` | Permitido en `BORRADOR`, `PENDIENTE` y `RECHAZADA`. |
...
| Acción | Método y ruta | Notas |
| ------ | ------------- | ----- |
| Listar citas pendientes | `GET /nutricionistas/:id/citas?estado=PENDIENTE` | Permite filtrar por rango de fechas. |
| Aprobar cita | `POST /citas/:id/aprobar` | Cambia a `APROBADA`. Dispara confirmación vía correo y push al paciente. |
...
| Acción | Método y ruta | Notas |
| ------ | ------------- | ----- |
| Crear cita confirmada | `POST /profesionales/:id/citas/confirmada` | Disponible para nutricionistas y administradores. La cita nace en `APROBADA` y dispara notificación inmediata al paciente. |
| Reprogramar cita aprobada | `POST /citas/:id/reprogramar` | Permite mover fecha/hora manteniendo estado `APROBADA`. Requiere comentario y notifica a las partes. |
...
- Notificar por correo y push (aprovechando la infraestructura existente) en cada transición relevante:
  ...
  - Envío a revisión (`BORRADOR` → `PENDIENTE`) al nutricionista.
  - Aprobación (`PENDIENTE` → `APROBADA`) al paciente.
  - Rechazo (`PENDIENTE` → `RECHAZADA`) al paciente con el comentario incluido.
  - Reprogramación o cancelación en cualquier estado a todos los involucrados.
  - Creación directa `APROBADA` por nutricionista/administrador al paciente.
...
- Tiempo promedio desde `PENDIENTE` hasta `APROBADA` o `RECHAZADA`.
