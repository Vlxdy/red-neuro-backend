...
Este módulo expone APIs para gestionar el personal de salud y sus ocupaciones asociadas. Se reutiliza la lógica de usuarios existente para altas/actualizaciones de datos personales y el rol `PERSONAL_SALUD`. El personal de salud no se elimina físicamente, solo puede activarse o inactivarse.

## Componentes principales

- **DTOs**: Definen la estructura de entrada para crear/actualizar profesionales y recibir IDs de ocupaciones.
- **Servicio**: Orquesta la creación y actualización de usuarios con rol `PERSONAL_SALUD`, y vincula ocupaciones.
- **Repositorio**: Consulta el personal de salud con paginación, obtiene detalles, y crea/reemplaza asociaciones de ocupaciones.
...
- **Descripción**: Crea un nuevo usuario con rol `PERSONAL_SALUD` y opcionalmente asigna ocupaciones.
...
- `idOcupaciones` es opcional. Si se omite, el profesional se crea sin ocupaciones.

### 4) Actualizar profesional de salud

- **Método**: `PATCH /personal-salud/:id`
- **Descripción**: Actualiza datos personales, correo, bandera de supervisor o reemplaza ocupaciones.
...
- Si `idOcupaciones` es enviado, se reemplazan todas las ocupaciones existentes por las nuevas.
- Si no se envía, las ocupaciones no cambian.
...
- **Manejo de IDs**: usar siempre `idOcupaciones` para enviar IDs de ocupación.
- **Sincronización de ocupaciones**: enviar la lista completa en actualizaciones si se requiere reemplazar.
